...
function getUserId(): string {
  ...
  let uid = localStorage.getItem('nyaya_user_id');
  if (!uid) {
    uid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (c) => {
      const r = (Math.random() * 16) | 0;
      return (c === 'x' ? r : (r & 0x3) | 0x8).toString(16);
    });
    localStorage.setItem('nyaya_user_id', uid);
  }
  return uid;
}
...
export default function QuizSystemPage() {
  ...
  // ── Fetch quizzes from proxy (which transforms backend shape) ──────────────
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        ...
        setFetchError('Could not load quizzes. Is the backend running?');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // ── Record attempt start on backend ───────────────────────────────────────
  const startAttempt = async (quizId: string) => {
    try {
      const res = await fetch('/api/attempts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'start', user_id: getUserId(), quiz_id: quizId }),
      });
      const data = await res.json();
  ...
  // ── Record attempt completion on backend ───────────────────────────────────
  const completeAttempt = async (finalScore: number) => {
    if (!attemptId) return;
    try {
      await fetch('/api/attempts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'complete', attempt_id: attemptId, score: finalScore }),
      });
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-grey-50 p-6 md:p-12 pb-24 relative overflow-hidden font-sans">
        ...
        <div className="w-full absolute top-0 left-0 right-0 z-20 flex items-center justify-between p-2.5 md:p-3.5 bg-white/80 backdrop-blur-md shadow-sm">
          ...
          <Link href="/history" className="w-11 h-11 md:w-12 md:h-12 bg-navy-900 text-white rounded-full hover:bg-navy-800 transition shadow-sm flex items-center justify-center" title="View History">
