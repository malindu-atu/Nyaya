...
export default function QuizDashboard() {
  const { userProfile, userStats, loading, formatTime, handleLogout, router } =
    useQuizDashboardLogic();

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        Loading Nyaya Dashboard...
      </div>
    );
  }
            ...
            <div className="w-full space-y-5 border-t border-gray-100 pt-6 text-left">
              ...
              <div className="w-full flex flex-col items-center gap-7 pt-6 border-t border-gray-100">
                ...
                <h3 className="text-lg font-bold text-slate-800 leading-tight">
                  {userStats?.average_score >= 80
                    ? "Excellent work, You're mastering the material. Keep pushing for that 100%"
                    : userStats?.average_score >= 50
                      ? "Good progress, You have a solid foundation. Consistency is the key to mastery."
                      : userStats?.average_score > 0
                        ? "Keep practicing, Every mistake is a learning opportunity. You've got this."
